...
## soil water balance equation               
def soil_moisture_simulation_R_ET_Z_v2(p,Ndays,s_start,R,ET_max,Zr):
    out=outputsim()
    s = [0] * (Ndays+1)
    s[0]=s_start
    vol_irr=[0] * (Ndays+1)
    ...
    for t in range(Ndays):
        ...
        ET_norm[t] = nerl(s[t],p);       
        dsdt[t] = R_norm[t] - ET_norm[t]
        if s[t]+dsdt[t]<p.s_tilde:
            I_norm[t] = ( p.s_target-(s[t]+dsdt[t]));
            I[t]=float(I_norm[t])*float(p.nZr);
            if len(t_irr)>1 :
                dt_irr.append(t-t_irr[len(t_irr)-1])
            t_irr.append(t)
        else:
...
## Yeld - dichotomic pocess (Vico2013)
def crop_development_simulation_R_ET_Z_v2(p,Ndays,s_start,R,ET_max,Zr):
    outs=soil_moisture_simulation_R_ET_Z_v2(p,Ndays,s_start,R,ET_max,Zr)
    s=outs.s
    ns=len(s)
    b = [0] * ns
    g = [0] * ns
